...
export const Sidebar = ({ conversations, activeId, onSelect, onBack }: SidebarProps) => {
    ...
    return (
        <div className="flex flex-col h-full">
            {/* Header */}
            <div className="h-16 bg-[#202c33] flex items-center justify-between px-4 border-b border-gray-700 shrink-0">
                <div className="flex items-center gap-2">
                    {onBack && (
                        <button onClick={onBack} className="p-2 hover:bg-gray-700 rounded-full mr-1" title="Back to Builder">
            ...
            {/* Search */}
            <div className="p-2 bg-[#111b21] border-b border-gray-700 shrink-0">
                <div className="bg-[#202c33] rounded-lg flex items-center px-3 py-1.5">
                    ...
                    <input
                        ...
                        className="bg-transparent border-none focus:ring-0 text-sm w-full ml-2 text-gray-200 placeholder-gray-500"
                    />
                </div>
            </div>
                ...
                {conversations.map((conv) => {
                    ...
                    return (
                        <div
                            ...
                            title={displayPhone}
                            className={`flex items-center p-3 cursor-pointer hover:bg-[#202c33] transition-colors ${activeId === conv.id ? "bg-[#2a3942]" : ""
                                }`}
                        >
                            <div className="w-12 h-12 rounded-full overflow-hidden mr-3 shrink-0 relative">
                                ...
                                {conv.isWindowOpen && (
                                    <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[#202c33]" title="24-hour window open"></div>
                                )}
                            </div>
                            <div className="flex-1 min-w-0 border-b border-gray-800 pb-3">
                                <div className="flex justify-between items-baseline mb-1">
                                    <h3 className="text-base font-normal text-gray-100 truncate">
                                        {displayName}
                                    </h3>
